...
import { getActiveUserCount, getCurrentState, replaceDocumentState } from '../yjsManager.js';
import { authRequired, requireDocumentRole } from '../middleware/authRequired.js';
...
router.get('/:id/snapshots', requireDocumentRole('id', 'viewer'), async (req, res) => {
  try {
    const result = await pool.query(
...
router.post('/:id/snapshots/:snapshotId/restore', requireDocumentRole('id', 'editor'), async (req, res) => {
  const client = await pool.connect();
  ...
  try {
    ...
    const hasUnsavedChanges = currentState && !statesEqual(currentState, latestSnapshot?.ydoc_state);
    ...
    await client.query('ROLLBACK').catch(() => {});
    console.error('[REST] POST /documents/:id/snapshots/:snapshotId/restore', err);
...
router.get('/:id/members', requireDocumentRole('id', 'viewer'), async (req, res) => {
  try {
    const result = await pool.query(
...
router.post('/:id/members', requireDocumentRole('id', 'owner'), async (req, res) => {
...
router.get('/:id/comments', requireDocumentRole('id', 'viewer'), async (req, res) => {
  try {
    const result = await pool.query(
...
router.post('/:id/comments', requireDocumentRole('id', 'viewer'), async (req, res) => {
  ...
  if (!Number.isFinite(anchorFrom) || !Number.isFinite(anchorTo) || anchorTo <= anchorFrom || !selectedText || !body) {
    return res.status(400).json({ error: 'Selected text and comment body are required' });
  }
  ...
  try {
    ...
    res.status(201).json({ ...result.rows[0], author_name: req.user.display_name });
  } catch (err) {
...
router.patch('/:id/comments/:commentId', requireDocumentRole('id', 'editor'), async (req, res) => {
  try {
    const result = await pool.query(
